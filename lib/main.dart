...
class _MyHomePageState extends ConsumerState<MyHomePage> with SingleTickerProviderStateMixin {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          FutureBuilder(
            future: ImageUtils.loadImageByFile("/data/user/0/com.example.flutter_snippet/cache/CAP2045022776505310768.jpg"),
            builder: (_, image) {
              if (image.connectionState == ConnectionState.done) {
                var data = image.data;
                debugPrint("wyn data11 is $data");
                if (null != data) {
                  return _showCroppedImage(data);
                  // return _showPainter(data);
                } else {
                  return Text("没有获取到图片啊 ${image.error}");
                }
              } else {
          ...
          //   var imageUrl = ref.watch(imageUrlProvider);
          //   debugPrint("imageUrl is $imageUrl");
          //   if (null == imageUrl) {
          //     return const Text("等待图片");
          //   } else {
          //     return FutureBuilder(
          //       future: ImageUtils.loadImageByFile(imageUrl),
          //       builder: (_, image) {
          //         if (image.connectionState == ConnectionState.done) {
          //           var data = image.data;
          //           debugPrint("wyn data11 is $data");
  ...
  Widget _showCroppedImage(ui.Image background) {
    return FutureBuilder(
      future: createCanvas(background),
      builder: (_, image) {
        if (image.connectionState == ConnectionState.done) {
          var data = image.data;
          debugPrint("wyn data22 is $data");
          if (null != data) {
            return _showPainter(data);
          } else {
            return Text("没有获取到图片啊22 ${image.error}");
          }
        } else {
          return const CircularProgressIndicator();
        }
  ...
  Widget _showPainter(ui.Image background) {
    return Container(
      color: Colors.cyan,
      width: double.infinity,
      height: 300,
      child: CustomPaint(
        painter: MyCustomPainter(background),
      ),
    );
  }

  Future<ui.Image> createCanvas(ui.Image background) async {
    ...
    debugPrint("wyn rect is $rect");
    debugPrint("wyn dstRect is $dstRect");
    canvas.drawImageRect(background, rect, dstRect, Paint());
    // canvas.drawImage(background, Offset(0, height / 3), Paint());
    // canvas.drawCircle(const Offset(100, 100), 50, Paint()..color=Colors.red);
    Picture picture = recorder.endRecording();
    ui.Image image = await picture.toImage(width.toInt(), height ~/ 3);

    debugPrint("wyn aaa image is $image");
    ...
    if (null != byteData) {
      try {
        ...
        debugPrint("wyn filePath is $filePath");
...
class _CameraExampleHomeState extends ConsumerState<CameraExampleHome> with WidgetsBindingObserver, TickerProviderStateMixin {
      ...
      children: <Widget>[
        Row(
          ...
          children: <Widget>[
            ...
            IconButton(
              icon: Icon(
                  controller?.value.isCaptureOrientationLocked ?? false ? Icons.screen_lock_rotation : Icons.screen_rotation),
              color: Colors.blue,
  ...
  void onTakePictureButtonPressed() {
    takePicture().then((XFile? file) {
      if (mounted) {
        ...
        ref.read(imageUrlProvider.notifier).state = file?.path;
      }
    });
  }
